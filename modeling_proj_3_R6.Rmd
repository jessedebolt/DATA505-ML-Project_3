...
raw_incomej = read_csv("openml_1590.csv", na=c("?"))

income2j = read_csv("openml_1590.csv", na=c("?")) %>%
  drop_na() %>%
  mutate(income_above_50k = class==">50K") %>%
  select(-class)

income2j$income_above_50k <- as.numeric(factor(income2j$income_above_50k, levels = unique(income2j$income_above_50k))) - 1
...
income2j$age_group <- factor(income2j$age_group, levels = c("young", "prime", "retired"))
...
#check if factor
class(income2j$income_above_50k)

# Convert 'income_above_50k' to a factor
income2j$income_above_50k <- as.factor(income2j$income_above_50k)
...
```{r}
income2j <- income2j %>% select(-fnlwgt)

unique(income2j$workclass)
...
income2j$hours_per_week <- factor(income2j$hours_per_week, levels = c("part_time", "full_time", "overtime"))
...
income2j$occupation_ordered <- factor(income2j$occupation, levels = occupation_order)
...
countries <- unique(income2j$`native-country`)
income2j$`native-country` <- ifelse(income2j$`native-country` %in% countries[-1], "Other", income2j$`native-country`)
...
income2j_dummies <- income2j %>%
  dummy_cols(select_columns = setdiff(names(income2j), "income_above_50k"), remove_selected_columns = T)
...
```{r results='hide'}
pr_income <- prcomp(x = select(income2j_dummies, -income_above_50k), scale = T, center = T)
...
```{r}
prc <- bind_cols(select(income2j_dummies,income_above_50k),as.data.frame(pr_income$x)) %>%
select(1:6) %>% 
...
```{r}
prc10 <- bind_cols(select(income2j_dummies,income_above_50k),as.data.frame(pr_income$x)) %>%
select(1:11)

prc25 <- bind_cols(select(income2j_dummies,income_above_50k),as.data.frame(pr_income$x)) %>%
select(1:26)

prcAll <- bind_cols(select(income2j_dummies,income_above_50k),as.data.frame(pr_income$x))
...
```{r}
confusionMatrix(predict(fit, income3j),factor(income3j$income_above_50k))
```
...
# Additional attempts
## Various models, additional EDA, etc.
