...
kclust <- kmeans(income, centers = 3)
kclust$centers
...
```{r}
incomek <- augment(kclust,income)
head(incomek)
```

##Visualize Clusters
```{r}
#need to do more of these and change the scale
incomek %>% 
...
kclusts <- tibble(k = 1:6) %>%
  mutate(
    kclust = map(k, ~kmeans(income, .x)),
    glanced = map(kclust, glance),
    augmented = map(kclust, augment, income)
  )
#with 1:9 get: Warning: Quick-TRANSfer stage steps exceeded maximum (= 2261100)
...
#need to changed from price points, but still not right
ggplot(assignments, aes(age, 'education-num')) +
...
```{r}
#can still look for elbow (looks like about 4)
clusterings <- kclusts %>%
