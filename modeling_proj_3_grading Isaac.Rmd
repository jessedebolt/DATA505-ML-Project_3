...
#income2 <- income2 %>% select(-`workclass_Without-pay`)
...
##Occupation
```{r}
ggplot(income2, aes(x = occupation, fill = as.factor(income_above_50k))) +
  geom_bar(position = "dodge") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
# Create dummy variables for all categorical columns except 'income_above_50k'
income2_dummies <- income2 %>%
  dummy_cols(select_columns = setdiff(names(income2), "income_above_50k"), remove_selected_columns = T)

# View the dataset with dummy variables
head(income2_dummies)
